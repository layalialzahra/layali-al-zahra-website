...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!setupToken || setupToken.length < 32 || !timingSafeToken(providedToken, setupToken)) {
      return res.status(403).json({ success: false, message: "Invalid setup authorization" });
    }
    ...
    const admin = { _id: result.insertedId, username };
