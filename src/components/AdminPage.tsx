...
export default function AdminPage() {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      setStatus('login'); setAdminUsername(''); setPassword('');
  ...
  return (
    <div className="min-h-screen bg-[#faf8f5]">
      <header className="border-b border-stone-200 bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4"><div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5" /><div><p className="font-semibold">Layali Al Zahra</p><p className="text-xs text-stone-500">Admin Dashboard</p></div></div><Button variant="outline" onClick={handleLogout} disabled={busy}><LogOut className="mr-2 h-4 w-4" />Logout</Button></div></header>
      <main className="mx-auto max-w-7xl px-6 py-10"><div className="mb-8"><p className="text-sm text-stone-500">Signed in as</p><h1 className="text-3xl font-semibold">{adminUsername}</h1></div><div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">{[['Content','Manage Blog, Tips & News'],['Website','Manage Services & Packages'],['Media','Manage Gallery & Images'],['Settings','SEO, Contact & Account']].map(([title, desc]) => <Card key={title}><CardHeader><CardTitle className="text-lg">{title}</CardTitle></CardHeader><CardContent><p className="text-sm text-stone-500">{desc}</p><p className="mt-4 text-xs text-stone-400">CMS module coming in the next stage.</p></CardContent></Card>)}</div></main>
    </div>
  );
}
